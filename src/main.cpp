...
#if defined(NDEBUG)
#error "emrals cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uemrals) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateEMRALSSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating EMRALS supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating EMRALS supply..."), percent);
		}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight <= 1000) {
        ...
        mnSubsidy = 0.0064 * COIN;
    } else if (nHeight <= 30000) {
        mnSubsidy = 0.0385 * COIN;
    } else if (nHeight <= 40000) {
        mnSubsidy = 0.05 * COIN;
    } else if (nHeight <= 60000) {
        mnSubsidy = 0.09 * COIN;
        ...
        mnSubsidy = 11.90 * COIN;
    } else if (nHeight <= 400000) {
        mnSubsidy = 13.90 * COIN;
    } else if (nHeight <= 450000) {
        mnSubsidy = 15.90 * COIN;
    } else if (nHeight <= 500000) {
        mnSubsidy = 17.90 * COIN;
    } else if (nHeight <= 550000) {
        mnSubsidy = 19.90 * COIN;
    } else if (nHeight <= 600000) {
        mnSubsidy = 24.90 * COIN;
    } else if (nHeight <= 650000) {
        mnSubsidy = 26.90 * COIN;
    } else {
        mnSubsidy = 34.00 * COIN;
    }
...
int64_t GetDevPayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight <= 1000) { 
        ...
        nDevPayment = 0.0008 * COIN;
    } else if (nHeight <= 30000) {
        nDevPayment = 0.00075 * COIN;
    } else if (nHeight <= 40000) {
        nDevPayment = 0.005 * COIN;
    } else if (nHeight <= 60000) {
        nDevPayment = 0.005 * COIN;
    } else if (nHeight <= 100000) {
        nDevPayment = 0.02 * COIN;
    } else if (nHeight <= 125000) {
        nDevPayment = 0.04 * COIN;
    } else if (nHeight <= 150000) {
        nDevPayment = 0.045 * COIN;
    } else if (nHeight <= 650000) {
        nDevPayment = 0.05 * COIN;
    } else {
        nDevPayment = 0.50 * COIN;
    }
...
{
    util::ThreadRename("emrals-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> emralsInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                emralsInputs.push_back(stakeIn);
        }
        const bool hasEMRALSInputs = !emralsInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasEMRALSInputs) {
                    ...
                    for (const CTxIn& emralsIn : emralsInputs)
                        if(emralsIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasEMRALSInputs) {
                            for (const CTxIn& stakeIn : emralsInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // emrals: We use certain sporks during IBD, so check to see if they are
