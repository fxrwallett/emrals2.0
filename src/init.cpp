...
void PrepareShutdown()
{
    ...
    util::ThreadRename("emrals-shutoff");
...
void OnRPCPreCommand(const CRPCCommand& cmd)
{
    ...
    if (strWarning != "" && !GetBoolArg("-disablesafemode", false) &&
        !cmd.okSafeMode)
        throw JSONRPCError(RPC_FOREMRALSDEN_BY_SAFE_MODE, std::string("Safe mode: ") + strWarning);
}
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "emrals.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "emralsd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 31472, 18352));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all emrals specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in EMRALS) allowed by GUI and RPC for the stake split threshold (default: %s)"),
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The EMRALS developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("emrals-loadblk");
...
/** Sanity checks
 *  Ensure that emrals is running in a usable environment with all
...
bool AppInit2()
{
 ...
 /*   if (GetBoolArg("-masternode", false) && !GetBoolArg("-listen", true))
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. emrals is shutting down."));
...
#endif
    // Make sure only a single emrals process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. emrals is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // emrals: load previous sessions sporks if we have them.
                ...
                int chainHeight;
                {
                    ...
                    // initialize EMRALS supply to 0
                    nMoneySupply = 0;

                    // Load EMRALS supply from DB
                    if (chainHeight >= 0) {
                        const uint256& tipHash = chainActive[chainHeight]->GetBlockHash();
                        CLegacyBlockIndex bi;

                        // Load EMRALS supply amount
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of emrals") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart emrals to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_emrals && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_emrals);
            }
