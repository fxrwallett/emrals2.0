...
bool CEMRALSStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CEMRALSStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CEMRALSStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CEMRALSStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CEMRALSStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CEMRALSStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CEMRALSStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CEMRALSStake::GetUniqueness() const
{
    //The unique identifier for a EMRALS stake is the outpoint
...
CBlockIndex* CEMRALSStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CEMRALSStake::ContextCheck(int nHeight, uint32_t nTime)
{
