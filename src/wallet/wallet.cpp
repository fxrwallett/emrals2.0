...
/**
 * Fees smaller than this (in uemrals) are considered zero fee (for transaction creation)
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                } else if (pcoin->vout[i].nValue == GetMNCollateral(chainActive.Height()) * COIN
                    && chainActive.Height() > Params().GetConsensus().nCollateralMaturityEnforcementHeight
                    && nDepth < Params().GetConsensus().nCollateralMaturity) {
                    //LogPrintf("AvailableCoins locked MN reward: skip %d / %d / %d \n", nDepth, pcoin->vout[i].nValue, chainActive.Height());
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 EMRALS collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 EMRALS collateral
