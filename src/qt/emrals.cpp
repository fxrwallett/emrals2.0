...
#include "qt/emrals/splash.h"
#include "qt/emrals/welcomecontentwidget.h"
...
/** Class encapsulating emrals startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (emralsGUI)
...
{
    window = new emralsGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &emralsGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &emralsGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(emralsGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(emralsGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &emralsGUI::handlePaymentRequest);
        connect(window, &emralsGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Emrals can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Emrals"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Emrals"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Emrals didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
