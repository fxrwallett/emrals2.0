...
#include "qt/emrals/forms/ui_send.h"
#include "qt/emrals/addnewcontactdialog.h"
#include "qt/emrals/qtutils.h"
#include "qt/emrals/sendchangeaddressdialog.h"
#include "qt/emrals/optionbutton.h"
#include "qt/emrals/sendconfirmdialog.h"
#include "qt/emrals/myaddressrow.h"
#include "qt/emrals/guitransactionsutils.h"
...
SendWidget::SendWidget(emralsGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-emrals");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination emralsAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(emralsAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(emralsAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
