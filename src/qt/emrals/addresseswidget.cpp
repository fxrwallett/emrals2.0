...
#include "qt/emrals/forms/ui_addresseswidget.h"
#include "qt/emrals/addresslabelrow.h"
#include "qt/emrals/addnewaddressdialog.h"
#include "qt/emrals/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(emralsGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination emralsAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(emralsAdd, label.toUtf8().constData(),
