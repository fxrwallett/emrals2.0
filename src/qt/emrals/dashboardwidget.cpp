...
#include "qt/emrals/dashboardwidget.h"
#include "qt/emrals/forms/ui_dashboardwidget.h"
#include "qt/emrals/sendconfirmdialog.h"
#include "qt/emrals/txrow.h"
...
DashboardWidget::DashboardWidget(emralsGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareEMRALS, "square-chart-emrals");
    setCssProperty(ui->labelEMRALS, "text-chart-emrals");
    ...
    setCssProperty(ui->labelAmountEMRALS, "text-stake-emrals-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &emralsGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &emralsGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &emralsGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            emrals = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalEMRALS += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEMRALS.append(emrals);

        int max = std::max(emrals, emrals);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesEMRALS);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEMRALS > 0) {
        setCssProperty(ui->labelAmountEMRALS, "text-stake-emrals");
    } else {
        setCssProperty(ui->labelAmountEMRALS, "text-stake-emrals-disable");
    }
    forceUpdateStyle({ui->labelAmountEMRALS});
    ui->labelAmountEMRALS->setText(GUIUtil::formatBalance(chartData->totalEMRALS, nDisplayUnit));
