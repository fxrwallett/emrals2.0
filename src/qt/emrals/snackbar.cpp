...
#include "qt/emrals/forms/ui_snackbar.h"
#include "qt/emrals/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(emralsGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &emralsGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
