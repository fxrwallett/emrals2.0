...
*Setup instructions for a gitian build of emrals using a VM or physical system.*
...
to the emrals GitHub release page.
...
- [Building emrals](#building-emrals-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/emrals-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/emrals-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/emrals-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/emrals-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/emrals-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/emrals-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/emralsvps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@emrals.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/emrals-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/emrals-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/emrals-osx-*-build.assert
```
