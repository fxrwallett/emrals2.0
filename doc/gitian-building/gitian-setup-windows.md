...
The build script we'll be using is contained in the emrals github repository ([contrib/gitian-build.py](https://github.com/emralsvps-project/emrals/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the emrals repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/emralsvps-project/emrals/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, emrals-detached-sigs, and emrals GitHub repos.
